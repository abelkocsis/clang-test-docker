...
if [ $? -ne 0 ]; then
    printf "Installing dependencies failed. Please, try again.\n"
    exit 5
fi
...
for p in "${projects[@]}" ; do
    ...
    if [ -f "./requirements/"$p"_custom_deps_debian.sh" ]; then
        ...
        if [ $? -ne 0 ]; then
            printf "Installing dependencies failed. Check your custom dependency file for $p.\n"
            exit 6
        fi
        ...
        if [ $? -ne 0 ]; then
            printf "Installing dependencies failed. Make sure that you used the right dependency name when setting up $p.\n"
            exit 7
        fi
    fi
done